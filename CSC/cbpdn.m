function [Y, SCOptsinf] = cbpdn(D, S, lambda, SCOpts)
...
%
% Usage:
%       [Y, SCOptsinf] = cbpdn(D, S, lambda, SCOpts);
%
% Input:
...
if isempty(SCOpts.RhoRsdlTarget),
  if SCOpts.StdResiduals,
    ...
    SCOpts.RhoRsdlTarget = 1 + (18.3).^(log10(lambda) + 1);
  end
end
...
% Main loop
k = 1;
while k <= SCOpts.MaxMainIter && (r > epri | s > edua),
  ...
  if SCOpts.RelaxParam == 1,
    ...
    Xr = SCOpts.RelaxParam*X + (1-SCOpts.RelaxParam)*Y;
  end

  % Solve Y subproblem
  Y = shrink(Xr + U, (lambda/rho)*SCOpts.L1Weight);
  if SCOpts.NonNegCoef,
    Y(Y < 0) = 0;
  end
  ...
  if SCOpts.AuxVarObj,
    ...
    Jl1 = sum(abs(vec(bsxfun(@times, SCOpts.L1Weight, Y))));
  else
    Jdf = sum(vec(abs(sum(bsxfun(@times,Df,Xf),3)-Sf).^2))/(2*xsz(1)*xsz(2));
    Jl1 = sum(abs(vec(bsxfun(@times, SCOpts.L1Weight, X))));
  end
  Jfn = Jdf + lambda*Jl1;
  ...
  if SCOpts.StdResiduals,
    ...
    epri = sqrt(Nx)*SCOpts.AbsStSCOptsol+max(nX,nY)*SCOpts.RelStSCOptsol;
    edua = sqrt(Nx)*SCOpts.AbsStSCOptsol+rho*nU*SCOpts.RelStSCOptsol;
  else
  ...
  tk = toc(tstart);
  SCOptsinf.itstat = [SCOptsinf.itstat; [k Jfn Jdf Jl1 r s epri edua rho tk]];
  if SCOpts.Verbose,
    if SCOpts.AutoRho,
  ...
  if SCOpts.AutoRho,
    if k ~= 1 && mod(k, SCOpts.AutoRhoPeriod) == 0,
      if SCOpts.AutoRhoScaling,
        rhomlt = sqrt(r/(s*SCOpts.RhoRsdlTarget));
        if rhomlt < 1, rhomlt = 1/rhomlt; end
        if rhomlt > SCOpts.RhoScaling, rhomlt = SCOpts.RhoScaling; end
      else
        rhomlt = SCOpts.RhoScaling;
      end
      rsf = 1;
      if r > SCOpts.RhoRsdlTarget*SCOpts.RhoRsdlRatio*s, rsf = rhomlt; end
      if s > (SCOpts.RhoRsdlRatio/SCOpts.RhoRsdlTarget)*r, rsf = 1/rhomlt; end
      rho = rsf*rho;
      U = U/rsf;
      if SCOpts.HighMemSolve && rsf ~= 1,
        C = bsxfun(@rdivide, Df, sum(Df.*conj(Df), 3) + rho);
      end
    end
  end
